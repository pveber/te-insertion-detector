...
module Pipeline = struct
  ...
  let genome_of_string = function
    | "droSim1" -> Ucsc_gb.genome_sequence `droSim1
    | path -> input path
...
|}
  ...
  let fastq_gz mode path : [`sanger] fastq gz workflow =
    let fq = input path in
    match mode with
    | `preview i ->
...
module Repo = struct
  ...
  let analysis_pipeline_for_te mode ~genome_index ({ id } as te) fq1 fq2 =
    let tep = Pipeline.te_positions ~te ~genome_index fq1 fq2 in
  ...
  let analysis_pipeline mode ~transposable_elements ~genome ~fq1 ~fq2 =
    let genome = Pipeline.genome_of_string genome in
    let genome_index = Bowtie2.bowtie2_build genome in
    let fq1 = Pipeline.fastq_gz mode fq1 in
    let fq2 = Pipeline.fastq_gz mode fq2 in
    let open Bistro_repo in
    List.map transposable_elements ~f:(fun te ->
        analysis_pipeline_for_te mode ~genome_index te fq1 fq2
      )
    |> List.concat
