...
let merge_insertions_from_all_samples inserts =
  List.map inserts ~f:(fun ins_by_sample ->
      ...
      |> List.sort ~compare:(fun (i1, _) (i2, _) -> Biotk.GLoc.compare i1 i2)
      |> List.group ~break:(fun (i1, _) (i2, _) ->
          match Biotk.GLoc.dist i1 i2 with
          | None -> true
          | Some d -> Int.abs d > 200
        )
      |> List.map ~f:(fun grp ->
          let barycenter =
            List.map grp ~f:(fun ((l : Biotk.GLoc.t), _) -> (float l.lo +. float l.hi) /. 2.)
            |> List.fold ~init:0. ~f:( +. )
...
let%pworkflow annotated_insertions spe te_library =
  let inserts = [%eval group_insertions_by_te spe te_library ] in
  let grouped_inserts = merge_insertions_from_all_samples inserts in
  let gtf = Biotk.Gtf.load [%path Species.annotation spe ] in
  ...
  let feature_map annotation f =
    ...
    |> Biotk.GAnnot.LSet.of_stream
  in
  let exon_map = feature_map genes Biotk.Gene.exons in
  let intron_map = feature_map genes Biotk.Gene.introns in
  let utr3_map = feature_map (Biotk.Gtf.Annotation.utr3' annotation) (fun r -> [ Biotk.Gtf.Record.loc r ]) in
  let utr5_map = feature_map (Biotk.Gtf.Annotation.utr5' annotation) (fun r -> [ Biotk.Gtf.Record.loc r ]) in
  let upstream_map = feature_map genes Biotk.(fun g ->
      List.map g.Gene.transcripts ~f:(fun t ->
          Gene.Transcript.upstream t 5_000)
    ) in
  let downstream_map = feature_map genes Biotk.(fun g ->
      List.map g.Gene.transcripts ~f:(fun t ->
          Gene.Transcript.upstream t 5_000)
    ) in
  let gene_map =
    ...
    |> CFStream.Stream.map ~f:(fun (id, g) ->
        Biotk.Gene.loc g, (id, g)
      )
    |> Biotk.GAnnot.LMap.of_stream
  in
  let samples = samples_for_species spe in
  ...
  Out_channel.with_file [%dest] ~f:(fun oc ->
      ...
      List.iter2_exn te_library grouped_inserts ~f:(fun te inserts ->
          List.iter inserts ~f:(fun (chrom, pos, samples_with_reads) ->
              let loc = Biotk.GLoc.{ chr = chrom ; lo = pos ; hi = pos + 1 } in
              ...
              in
              let genes =
                Biotk.GAnnot.LMap.intersecting_elems gene_map loc
              ...
              let region_type =
                List.map feature_maps ~f:(fun map ->
                    if Biotk.GAnnot.LSet.intersects map loc then "1" else "0"
                  )
                |> String.concat ~sep:"\t"
              in
...
{{string Scripts.te_insertion_clustering}}
insertion_correlation_clust("{{dep (annotated_insertions spe te_library)}}",
                            "{{dest}}")
|}]
